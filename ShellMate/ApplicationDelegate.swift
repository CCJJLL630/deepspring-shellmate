...
class ApplicationDelegate: NSObject, NSApplicationDelegate, SPUUpdaterDelegate {
  ...
  func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    NotificationCenter.default.addObserver(
      self, selector: #selector(handleStartAppInitialization), name: .startAppInitialization,
      object: nil)
  }
  ...
  func setupSentry() {  // Initialize Sentry SDK
    ...
    SentrySDK.start { options in
      options.dsn =
        "https://0256895de48160d74021d3ffe93688e6@o4507511162798080.ingest.us.sentry.io/4507540074463232"
  ...
  func getURLForTerminal() -> URL? {
    ...
    if let urls = LSCopyApplicationURLsForBundleIdentifier(
      terminalBundleIdentifier as CFString, nil)?.takeRetainedValue() as? [URL],
