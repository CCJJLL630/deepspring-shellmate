...
struct SettingsView: View {
  ...
  var body: some View {
    TabView {
      GeneralView(
        appViewModel: appViewModel, licenseViewModel: licenseViewModel,
  ...
  var body: some View {
    HStack {
      ...
      LaunchAtLogin.Toggle {
        Text("Launch ShellMate at login")
          .font(.body)
      }
    }
  }
  ...
  var body: some View {
    HStack {
      ...
      Toggle(isOn: $companionModeManager.isCompanionModeEnabled) {
        Text("Auto-open \(companionModeManager.appName) when you open terminal")
          .font(.body)
      }
  ...
  var body: some View {
    HStack {
      Text("Window Position")
        .frame(width: 150, alignment: .trailing)
      Picker(
        selection: $generalViewModel.windowAttachmentPosition,
        label: HStack {
      ...
      HStack(alignment: .top) {
        ...
        VStack(alignment: .leading, spacing: 0) {
          Text("If you don't have an API key yet, you can sign up for one at")
            ...
            .foregroundColor(Color.Text.gray)
          Link(
            "OpenAI - API Keys", destination: URL(string: "https://platform.openai.com/api-keys")!
          )
          .font(.footnote)
          .underline()
          .foregroundColor(Color.Text.gray)
